...
#include "ff_type.hpp"
#include "incremental_absorbing_relation_searcher.hpp"
#include "io.hpp"
...
void writeResult(
    ...
    if (!out.is_open()) throw std::runtime_error("Cannot open output: " + path);
        ...
        out << "# generator = incremental_absorbing_poly_searcher\n";
    out << "# p = " << cfg.p << "\n";
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        IncrementalAbsorbingRelationSearcher<FlintMod> searcher(input, options);
        const auto result = searcher.search();
        writeResult(argv[4], cfg, result);
        std::cerr << "[incremental_absorbing_poly_searcher] G size = " << G.size()
