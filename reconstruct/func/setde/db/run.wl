...
loopmom = {l1, l2};
extmom = {p1, p2, p3};
numrep = {s -> 1, t -> -2/3, msq -> 1/5, d -> 4 - 2*(1/7)};
sprep = {p1^2 -> 0, p2^2 -> 0, p3^2 -> msq, p1 p2 -> s/2, p1 p3 -> (t - msq)/2, p2 p3 -> (msq - s - t)/2} /. numrep;
pdlist = {l1^2, (l1 + p1)^2, (l1 + p1 + p2)^2, l2^2 - msq, (l2 - p1 - p2)^2 - msq, (l1 + l2)^2 - msq} /. numrep;
...
cfg = StringRiffle[{"B = 3", "N = 6", "p = " <> ToString[p], "d = " <> ToString[myMod[(d /. numrep), p]], "a = " <> ToString[myMod[a, p]], "b = " <> ToString[myMod[b, p]]}, "\n"];
Export["config", cfg <> "\n", "Text"];
